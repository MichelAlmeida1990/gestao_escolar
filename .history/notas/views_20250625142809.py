...
def boletim_aluno(request, aluno_id):
    ...
    # Obter turma atual do aluno através de TurmaAluno
    from turmas.models import TurmaAluno
    turma_aluno = TurmaAluno.objects.filter(aluno=aluno, ativo=True).first()
    turma_atual = turma_aluno.turma if turma_aluno else None
    
    # Estrutura para armazenar dados do boletim
    disciplinas = {}
    
    # Obter todas as disciplinas que o aluno tem notas
    disciplinas_aluno = Disciplina.objects.filter(
        avaliacoes__notas__aluno=aluno
    ).distinct()
...
def desempenho_turma(request, turma_id):
    ...
    if not request.user.is_staff and not hasattr(request.user, 'professor'):
        try:
            ...
            # Verificar se algum aluno do responsável está na turma através de TurmaAluno
            from turmas.models import TurmaAluno
    ...
    # Obter alunos da turma através de TurmaAluno
    alunos = Aluno.objects.filter(
        turmas_matriculadas__turma=turma,
        turmas_matriculadas__ativo=True
    )
    ...
    for disciplina in disciplinas:
        ...
        media_turma = Nota.objects.filter(
            avaliacao__disciplina=disciplina,
            aluno__turmas_matriculadas__turma=turma,
            aluno__turmas_matriculadas__ativo=True
