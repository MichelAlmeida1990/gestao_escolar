...
from responsaveis.models import Responsavel, RelacaoAlunoResponsavel
...
class DisciplinaListView(LoginRequiredMixin, StaffOrFilteredByResponsavelMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if hasattr(self.request.user, 'professor'):
            return queryset.filter(professor=self.request.user.professor)
        ...
        try:
            ...
            alunos_ids = RelacaoAlunoResponsavel.objects.filter(
                responsavel=responsavel
            ).values_list('aluno_id', flat=True)
            
            # Obtém as turmas desses alunos
            turmas_ids = Aluno.objects.filter(
                id__in=alunos_ids
            ).values_list('turma_id', flat=True)
            
            # Retorna apenas disciplinas dessas turmas
            return queryset.filter(avaliacao__turma_id__in=turmas_ids).distinct()
...
class AvaliacaoListView(LoginRequiredMixin, StaffOrFilteredByResponsavelMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if hasattr(self.request.user, 'professor'):
            return queryset.filter(disciplina__professor=self.request.user.professor)
...
def boletim_aluno(request, aluno_id):
    ...
    if not request.user.is_staff and not hasattr(request.user, 'professor'):
        try:
            responsavel = Responsavel.objects.get(usuario=request.user)
            if not RelacaoAlunoResponsavel.objects.filter(
                responsavel=responsavel, 
                aluno_id=aluno_id
            ).exists():
