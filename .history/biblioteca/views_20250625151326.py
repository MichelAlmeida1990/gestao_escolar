...
class EmprestimoListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if data_inicio:
            try:
                ...
                queryset = queryset.filter(data_emprestimo__date__gte=data_inicio_parsed)
            except ValueError:
                pass
        ...
        if data_fim:
            try:
                ...
                queryset = queryset.filter(data_emprestimo__date__lte=data_fim_parsed)
            except ValueError:
                pass
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            'search': self.request.GET.get('search', ''),
            'status_selecionado': self.request.GET.get('status', ''),
            'tipo_usuario_selecionado': self.request.GET.get('tipo_usuario', ''),
            'data_inicio': self.request.GET.get('data_inicio', ''),
            'data_fim': self.request.GET.get('data_fim', ''),
            'status_choices': Emprestimo.STATUS_CHOICES,
            'tipo_usuario_choices': Emprestimo.TIPO_USUARIO_CHOICES,
        })
        return context
...
def relatorio_emprestimos(request):
    """View para relatório detalhado de empréstimos"""
    # Filtros
    ...
    # Query base
    emprestimos = Emprestimo.objects.select_related('livro', 'aluno', 'professor')
    ...
    if tipo_usuario:
        emprestimos = emprestimos.filter(tipo_usuario=tipo_usuario)
    ...
    emprestimos_devolvidos = emprestimos.filter(status='devolvido').count()
    emprestimos_atrasados = emprestimos.filter(status='atrasado').count()
    
    # Valor total de multas
    valor_total_multas = emprestimos.aggregate(
    ...
    return render(request, 'biblioteca/relatorio_emprestimos.html', context)
