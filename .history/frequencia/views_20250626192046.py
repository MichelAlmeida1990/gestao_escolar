...
def aprovar_justificativa(request, justificativa_id):
    ...
    if acao in ['aprovar', 'reprovar']:
        if acao == 'aprovar':
            ...
            justificativa.save()  # Salvar após definir os campos
...
@login_required
def teste_aprovacao(request):
    """View de teste para debugar aprovações"""
    if not request.user.is_staff:
        messages.error(request, 'Apenas administradores podem acessar esta página.')
        return redirect('frequencia:index')
    
    justificativas = JustificativaFalta.objects.all().select_related(
        'registro_frequencia__aluno', 'registro_frequencia__turma', 'registro_frequencia__disciplina'
    ).order_by('-data_justificativa')
    
    context = {
        'justificativas': justificativas,
    }
    
    return render(request, 'frequência/teste_aprovacao.html', context)
