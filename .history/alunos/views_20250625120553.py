...
class StaffOrFilteredByResponsavelMixin(UserPassesTestMixin):
    """Mixin para verificar se o usuÃ¡rio Ã© staff ou responsÃ¡vel e filtrar alunos adequadamente"""
    
    def test_func(self):
        # Sempre permite acesso, mas o queryset serÃ¡ filtrado em get_queryset
        return True
    ...
    def get_queryset(self):
        ...
        # Se for responsÃ¡vel, filtra pelos alunos vinculados
        try:
            ...
            # Se nÃ£o for responsÃ¡vel nem staff, nÃ£o mostra nenhum aluno
...
class AlunoDetailView(LoginRequiredMixin, StaffOrFilteredByResponsavelMixin, DetailView):
    ...
    def get_object(self, queryset=None):
        ...
        # Se nÃ£o for staff, verificar se o aluno estÃ¡ vinculado ao responsÃ¡vel
        if not self.request.user.is_staff:
            try:
                responsavel = Responsavel.objects.get(usuario=self.request.user)
                if not ResponsavelAluno.objects.filter(responsavel=responsavel, aluno=obj).exists():
                    raise Http404("VocÃª nÃ£o tem permissÃ£o para visualizar este aluno.")
            except Responsavel.DoesNotExist:
                raise Http404("VocÃª nÃ£o tem permissÃ£o para visualizar este aluno.")
...
class AlunoDeleteView(LoginRequiredMixin, UserPassesTestMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        messages.success(request, 'Aluno excluÃ­do com sucesso!')
        return super().delete(request, *args, **kwargs)

def dashboard(request):
    if request.user.is_staff:
        # Administradores veem estatÃ­sticas de todos os alunos
        ...
        # ResponsÃ¡veis veem estatÃ­sticas apenas dos seus alunos
        try:
...
def meus_alunos(request):
    """Exibe apenas os alunos vinculados ao responsÃ¡vel logado"""
    ...
    try:
        ...
        messages.error(request, 'VocÃª nÃ£o estÃ¡ cadastrado como responsÃ¡vel.')
