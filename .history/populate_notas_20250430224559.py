...
from turmas.models import Turma  # Importando o modelo Turma
...
def verificar_modelos():
    ...
    print("\nVerificando se existem turmas cadastradas:")
...
def criar_turmas_exemplo():
    ...
    # Criar algumas turmas básicas
    for serie in range(1, 4):  # 1º, 2º e 3º ano
        for letra in ['A', 'B', 'C']:
            nome = f"{serie}º Ano {letra}"
            ...
            turmas_exemplo.append(turma)
            print(f"Turma criada: {turma.nome}")
    
    return turmas_exemplo
...
def criar_avaliacoes(disciplinas):
    ...
    # Obter todas as turmas existentes
    turmas = list(Turma.objects.all())
    if not turmas:
        print("Não há turmas cadastradas no sistema. Criando turmas de exemplo...")
        # Criar algumas turmas de exemplo se não existirem
        turmas = criar_turmas_exemplo()
        
    if not turmas:
        print("Não foi possível criar turmas. Abortando.")
        return []
    
    periodos = ["Manhã", "Tarde", "Noite"]
    
    # Limitar a 3 disciplinas para evitar sobrecarga
    disciplinas_limitadas = disciplinas[:3] if len(disciplinas) > 3 else disciplinas
    
    for disciplina in disciplinas_limitadas:
        # Cria apenas 2 avaliações por disciplina para evitar sobrecarga
        num_avaliacoes = 2
        ...
        for i in range(num_avaliacoes):
            ...
            avaliacao = Avaliacao.objects.create(
                ...
                turma=turmas[0],  # Usando a primeira turma para simplificar
                tipo=tipo,
...
def lancar_notas(avaliacoes):
    ...
    # Obter todos os alunos (limitando a 10 para evitar sobrecarga)
    alunos = list(Aluno.objects.all()[:10])
    if not alunos:
        print("Não há alunos cadastrados no sistema.")
        return []
    
    # Limitar o número de avaliações para evitar sobrecarga
    avaliacoes_limitadas = avaliacoes[:5] if len(avaliacoes) > 5 else avaliacoes
    
    contador = 0
    max_notas = 50  # Limite máximo de notas para evitar loops infinitos
    
    for avaliacao in avaliacoes_limitadas:
        ...
        for aluno in alunos:
            ...
            if contador >= max_notas:
                print(f"Limite de {max_notas} notas atingido. Parando o processo.")
