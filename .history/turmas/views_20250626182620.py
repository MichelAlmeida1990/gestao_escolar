...
from .models import Turma, TurmaAluno, TurmaProfessor
...
class TurmaDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['alunos'] = Aluno.objects.filter(turmas_matriculadas__turma=turma, turmas_matriculadas__ativo=True)
        context['professores'] = turma.professores_vinculados.all()
        return context
...
def adicionar_alunos(request, turma_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for aluno in alunos:
                TurmaAluno.objects.get_or_create(turma=turma, aluno=aluno)
            messages.success(request, f'{len(alunos)} aluno(s) adicionado(s) à turma com sucesso!')
            return redirect('turmas:turma_detail', pk=turma.id)
    else:
        # Excluir alunos que já estão na turma
        alunos_ja_matriculados = Aluno.objects.filter(turmas_matriculadas__turma=turma, turmas_matriculadas__ativo=True)
        alunos_disponiveis = Aluno.objects.exclude(id__in=alunos_ja_matriculados.values_list('id', flat=True))
        form = AlunoTurmaForm()
        form.fields['alunos'].queryset = alunos_disponiveis
    
    # Calcular informações da turma
    alunos_ativos_count = TurmaAluno.objects.filter(turma=turma, ativo=True).count()
    vagas_restantes = turma.capacidade - alunos_ativos_count
...
def remover_aluno(request, turma_id, aluno_id):
    ...
    if request.method == 'POST':
        TurmaAluno.objects.filter(turma=turma, aluno=aluno).update(ativo=False)
